    ...
    try {
      ...
      if (error) {
        if (error.message.includes("chat_sessions") || error.message.includes("schema cache") || error.message.includes("does not exist")) {
          return NextResponse.json({ success: true, useLocalFallback: true, sessionId: id, status: "needs_table" });
        }
        console.log("Notice: Supabase chat_sessions upsert fallback:", error.message);
